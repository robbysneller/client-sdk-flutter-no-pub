...
const defaultRetryDelaysInMs = [0, 300, 2 * 2 * 300, 3 * 3 * 300, 4 * 4 * 300, maxRetryDelay, maxRetryDelay, maxRetryDelay, maxRetryDelay, maxRetryDelay];
...
class Engine extends Disposable with EventsEmittable<EngineEvent> {
  ...
  rtc.RTCDataChannel? get dataChannel => _subscriberPrimary ? _reliableDCSub ?? _lossyDCSub : _reliableDCPub ?? _lossyDCPub;
  ...
  Engine({required this.roomOptions, SignalClient? signalClient, PeerConnectionCreate? peerConnectionCreate})
  ...
  Future<void> connect(
    ...
    try {
      ...
      await events.waitFor<EngineJoinResponseEvent>(
        ...
        onTimeout: () => throw ConnectException('Timed out waiting for SignalJoinResponseEvent', reason: ConnectionErrorReason.Timeout),
      );
      ...
      await events.waitFor<EnginePeerStateUpdatedEvent>(
        ...
        duration: this.connectOptions.timeouts.connection,
        onTimeout: () => throw MediaConnectException('Timed out waiting for PeerConnection to connect, please check your network for ice connectivity'),
      );
      events.emit(const EngineConnectedEvent());
    } catch (error) {
      logger.fine('Connect Error $error');

      events.emit(EngineDisconnectedEvent(reason: DisconnectReason.joinFailure));
      rethrow;
    }
  }
  ...
  @internal
  Future<void> sendDataPacket(lk_models.DataPacket packet) async {
    //
    ...
    if (_subscriberPrimary) {
      ...
      if (_publisherDataChannelState(packet.kind.toSDKType()) != rtc.RTCDataChannelState.RTCDataChannelOpen) {
        logger.fine('Waiting for data channel ${reliability} to open...');
        await events.waitFor<PublisherDataChannelStateUpdatedEvent>(filter: (event) => event.type == reliability, duration: connectOptions.timeouts.connection);
      }
    }
  ...
  Future<void> _createPeerConnections(RTCConfiguration rtcConfiguration) async {
    publisher = await Transport.create(_peerConnectionCreate, rtcConfig: rtcConfiguration, connectOptions: connectOptions);
    subscriber = await Transport.create(_peerConnectionCreate, rtcConfig: rtcConfiguration, connectOptions: connectOptions);
    ...
    subscriber?.pc.onConnectionState = (state) async {
      ...
      if (state.isDisconnected() || state.isFailed()) {
        await handleDisconnect(state.isFailed() ? ClientDisconnectReason.peerConnectionFailed : ClientDisconnectReason.peerConnectionClosed);
      }
    };
    ...
    subscriber?.pc.onTrack = (rtc.RTCTrackEvent event) {
      ...
      if (signalClient.connectionState == ConnectionState.reconnecting || signalClient.connectionState == ConnectionState.connecting) {
    ...
    try {
      ...
      _lossyDCPub?.stateChangeStream.listen(
        (state) => events.emit(PublisherDataChannelStateUpdatedEvent(isPrimary: !_subscriberPrimary, state: state, type: Reliability.lossy)),
      );
    ...
    try {
      ...
      _reliableDCPub?.stateChangeStream.listen(
        (state) => events.emit(PublisherDataChannelStateUpdatedEvent(isPrimary: !_subscriberPrimary, state: state, type: Reliability.reliable)),
      );
    } catch (err) {
      logger.severe('[$objectId] createDataChannel() did throw $err');
    }
      ...
      case _reliableDCLabel:
        ...
        _reliableDCSub?.stateChangeStream.listen(
          (state) => _reliableDCPub?.stateChangeStream.listen(
            (state) => events.emit(SubscriberDataChannelStateUpdatedEvent(isPrimary: _subscriberPrimary, state: state, type: Reliability.reliable)),
          ),
        );
        break;
      case _lossyDCLabel:
        ...
        _lossyDCSub?.stateChangeStream.listen(
          (event) => _reliableDCPub?.stateChangeStream.listen(
            (state) => events.emit(SubscriberDataChannelStateUpdatedEvent(isPrimary: _subscriberPrimary, state: state, type: Reliability.lossy)),
          ),
        );
        break;
      default:
  ...
  void _onDCMessage(rtc.RTCDataChannelMessage message) {
    ...
    if (dp.whichValue() == lk_models.DataPacket_Value.speaker) {
      ...
      // User packet
      events.emit(EngineDataPacketReceivedEvent(packet: dp.user, kind: dp.kind));
    } else if (dp.whichValue() == lk_models.DataPacket_Value.transcription) {
      // Transcription packet
      events.emit(EngineTranscriptionReceivedEvent(transcription: dp.transcription));
    } else if (dp.whichValue() == lk_models.DataPacket_Value.sipDtmf) {
      // SIP DTMF packet
      events.emit(EngineSipDtmfReceivedEvent(dtmf: dp.sipDtmf));
  ...
  Future<void> handleDisconnect(ClientDisconnectReason reason) async {
    ...
    events.emit(EngineAttemptReconnectEvent(attempt: reconnectAttempts! + 1, maxAttempts: _reconnectCount, nextRetryDelaysInMs: delay));
  ...
  Future<void> attemptReconnect(ClientDisconnectReason reason) async {
    ...
    if (_clientConfiguration?.resumeConnection == lk_models.ClientConfigSetting.DISABLED ||
        [ClientDisconnectReason.leaveReconnect, ClientDisconnectReason.negotiationFailed, ClientDisconnectReason.peerConnectionFailed].contains(reason)) {
      fullReconnectOnNext = true;
    }
    ...
    try {
      ...
      if (await signalClient.networkIsAvailable() == false) {
        ...
        await signalClient.events.waitFor<SignalConnectivityChangedEvent>(
          ...
          onTimeout:
              () => throw ConnectException('attemptReconnect: Timed out waiting for SignalConnectivityChangedEvent', reason: ConnectionErrorReason.Timeout),
        );
      }
      ...
      if (recoverable) {
        ...
        events.emit(EngineDisconnectedEvent(reason: DisconnectReason.disconnected));
        await cleanUp();
      }
    } finally {
      attemptingReconnect = false;
  ...
  Future<void> resumeConnection(ClientDisconnectReason reason) async {
    ...
    // wait for socket to connect rtc server
    await signalClient.connect(url!, token!, connectOptions: connectOptions, roomOptions: roomOptions, reconnect: true);

    await events.waitFor<SignalReconnectedEvent>(
      duration: connectOptions.timeouts.connection,
      onTimeout: () => throw ConnectException('resumeConnection: Timed out waiting for SignalReconnectedEvent', reason: ConnectionErrorReason.Timeout),
    );
    ...
    if (!isConnected) {
      ...
      await events.waitFor<EnginePeerStateUpdatedEvent>(
        filter: (event) => event.isPrimary && event.state.isConnected(),
        duration: connectOptions.timeouts.peerConnection,
        onTimeout: () => throw MediaConnectException('resumeConnection: Timed out waiting for EnginePeerStateUpdatedEvent'),
      );
  ...
  Future<void> restartConnection({String? regionUrl}) async {
    ...
    try {
      ...
      await connect(regionUrl ?? url!, token!, roomOptions: roomOptions, connectOptions: connectOptions, fastConnectOptions: fastConnectOptions);
  ...
  void _setUpSignalListeners() =>
      _signalListener
        ..on<SignalJoinResponseEvent>((event) async {
          ...
          var iceServersFromServer = event.response.iceServers.map((e) => e.toSDKType()).toList();
...
extension EnginePrivateMethods on Engine {
  ...
  rtc.RTCDataChannel? _publisherDataChannel(Reliability reliability) => reliability == Reliability.reliable ? _reliableDCPub : _lossyDCPub;
...
extension EngineInternalMethods on Engine {
  @internal
  Future<rtc.RTCRtpTransceiver> createTransceiverRTCRtpSender(LocalTrack track, PublishOptions opts, List<rtc.RTCRtpEncoding>? encodings) async {
    if (publisher == null) {
    ...
    var transceiverInit = rtc.RTCRtpTransceiverInit(direction: rtc.TransceiverDirection.SendOnly);
    if (encodings != null) {
      transceiverInit.sendEncodings = encodings;
    }
    final transceiver = await publisher!.pc.addTransceiver(
      track: track.mediaStreamTrack,
      kind: track is LocalVideoTrack ? rtc.RTCRtpMediaType.RTCRtpMediaTypeVideo : rtc.RTCRtpMediaType.RTCRtpMediaTypeAudio,
  ...
  @internal
  List<lk_rtc.DataChannelInfo> dataChannelInfo() => [_reliableDCPub, _lossyDCPub].nonNulls.where((e) => e.id != -1).map((e) => e.toLKInfoType()).toList();
